...
parser.add_option("--epoch", type=int, default=99, help="Max epoch")
...
parser.add_option('--load', action='store_true', default=False,help='Continue training')
#Model parameters
...
train = utils.LHCODataLoader(os.path.join(flags.folder,'LHCO','train_background_SB.h5'),flags.batch,hvd.rank(),hvd.size())
test = utils.LHCODataLoader(os.path.join(flags.folder,'LHCO','val_background_SB.h5'),flags.batch,hvd.rank(),hvd.size())
...
lr_schedule_body = keras.optimizers.schedules.CosineDecay(
    ...
    warmup_steps= 10*train.nevts//hvd.size()//flags.batch,
